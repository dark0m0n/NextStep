...
const MyHeader = () => {
    ...
    const submitSearch = () => {
        ...
        if (query) {
            console.log(`Пошук за запитом: "${query}" у: ${searchType}`);
            if (searchType === "project") {
                window.location.href = `/searchproj?query=${encodeURIComponent(query)}`;
            } else if (searchType === "employee") {
                window.location.href = `/searchemp?query=${encodeURIComponent(query)}`;
            }
        } else {
